...
void DrillClientImpl::handleHandshake(ByteBuf_t _buf,
    ...
    if(!error){
        ...
        for (int i=0; i<b2u.authenticationmechanisms_size(); i++) {
            this->m_serverAuthMechanisms.push_back(b2u.authenticationmechanisms(i));
        }
...
connectionStatus_t DrillClientImpl::authenticate(const DrillUserProperties* userProperties) {
    ...
    if (SASL_OK == m_saslResultCode) {
        DRILL_MT_LOG(DRILL_LOG(LOG_DEBUG) << "DrillClientImpl::authenticate: Successfully authenticated!"
                                          << std::endl;)
        ...
        DRILL_MT_LOG(DRILL_LOG(LOG_DEBUG) << "DrillClientImpl::authenticate: Authentication failed: "
                                          << m_saslResultCode << std::endl;)
        // shuts down socket as well
        return handleConnError(CONN_AUTH_FAILED, "User authentication failed. Check connection parameters?");
    }
}
