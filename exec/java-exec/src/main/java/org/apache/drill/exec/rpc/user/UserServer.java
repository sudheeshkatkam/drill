...
public class UserServer extends BasicServer<RpcType, UserClientConnectionImpl> {
  ...
  public UserServer(BootStrapContext context, BufferAllocator alloc, EventLoopGroup eventLoopGroup,
    ...
    this.worker = worker;
    this.alloc = alloc;
    this.loginManager = context.getLoginManager();
    final DrillConfig config = context.getConfig();
    authFactory = !config.getBoolean(ExecConstants.USER_AUTHENTICATION_ENABLED) ? null : context.getAuthFactory();
  ...
  public class UserClientConnectionImpl extends RemoteConnection implements UserClientConnection {
    ...
    public UserClientConnectionImpl(SocketChannel channel) {
      ...
      currentHandler = authFactory == null ? handler : new UserServerAuthenticationHandler(handler, loginManager);
    }
    ...
    SaslServer getSaslServer() throws IllegalStateException {
      if (saslServer == null) {
        throw new IllegalStateException("SASL server not initialized.");
      }
      return saslServer;
    }
    ...
    void finalizeSession() throws IOException {
      ...
      final String userName = loginManager.translateToLocalName(authorizationID);
