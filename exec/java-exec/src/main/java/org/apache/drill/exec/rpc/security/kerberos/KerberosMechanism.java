...
public class KerberosMechanism implements AuthenticationMechanism {
  ...
  public static final String SERVER_PRINCIPAL = "drill.exec.security.auth.principal";

  public static final String SERVER_KEYTAB_LOCATION = "drill.exec.security.auth.keytab";
  ...
  public KerberosMechanism(final DrillConfig config) throws DrillbitStartupException {
    final String servicePrincipal = config.getString(KerberosMechanism.SERVER_PRINCIPAL);
    final String keytab = config.getString(KerberosMechanism.SERVER_KEYTAB_LOCATION);

    try { // Kerberos mechanism requires a service to login
      final Configuration conf = new Configuration();
      conf.set(CommonConfigurationKeys.HADOOP_SECURITY_AUTHENTICATION, SIMPLE_NAME);
