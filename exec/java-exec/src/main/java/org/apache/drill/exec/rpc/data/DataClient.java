...
public class DataClient extends BasicClient<RpcType, DataClientConnection, BitClientHandshake, BitServerHandshake>{
  ...
  public DataClient(DrillbitEndpoint remoteEndpoint, BitConnectionConfigImpl config,
  ...
  protected void validateHandshake(BitServerHandshake handshake) throws RpcException {
    ...
    if (handshake.getAuthenticationMechanismsCount() != 0) { // remote requires authentication
      if (config.getAuthProvider() == null) {
        throw new RpcException(String.format("Drillbit (%s) requires auth, but auth is not configured.",
            remoteEndpoint.getAddress()));
      }
      if (!handshake.getAuthenticationMechanismsList().contains(config.getAuthMechanismToUse())) {
        throw new RpcException(String.format("Drillbit (%s) does not support %s", remoteEndpoint.getAddress(),
