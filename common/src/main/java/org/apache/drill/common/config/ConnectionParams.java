...
public final class ConnectionParams {
  ...
  public static final ImmutableSet<String> ALLOWED_BY_CLIENT =
      ImmutableSet.of(ZOOKEEPER_CONNECTION, DRILLBIT_CONNECTION, SCHEMA, USER, PASSWORD, IMPERSONATION_TARGET,
          AUTH_MECHANISM, PRINCIPAL, SERVICE_NAME, SERVICE_HOST, REALM);
  ...
  public boolean shouldAuthenticate() {
    return !Strings.isNullOrEmpty(properties.getProperty(PASSWORD)) ||
        !Strings.isNullOrEmpty(properties.getProperty(PRINCIPAL)) ||
        (!Strings.isNullOrEmpty(properties.getProperty(SERVICE_NAME))
            && !Strings.isNullOrEmpty(properties.getProperty(SERVICE_HOST)));
  }
